...
macroExpand state (_, List _ [Atom _ "gensym"], _) =
  return (state {gensymCounter = gensymCounter state + 1}, atom (show $ gensymCounter state))
macroExpand state (_, List _ [Atom _ "gensym", String prefix], _) =
...
macroExpand state z@(env, List _ (Atom _ "let" : bindsAndExpr), _) = do
  ...
  where
    ...
    isMacro (_, List _ (Atom _ "macro" : _)) = True
    isMacro _ = False
    varsWithoutMacros binds = (not . isMacro) `filter` vars binds
macroExpand state z@(env, List _ (Atom _ function : args), _) = do
...
macroExpand state (env, List _ args, _) = do
